"""
The following code was copied from abonello's GitHub repo at the following link:
https://github.com/abonello/riddle-game-app.
The code was then modified for the similarities game.
"""
...
all_similarities = []       # All similarities available for the game
similarity_counter = 0
attempt = 1                 # There are three attempts per similarity.
points = 10
gained_points = 0
wrong_answers =[]           # This will hold wrong answers
answer = ""                 # Current answer of current similarity
...
def sort_current_similarity(data):
    ''' Sort the data so that it is always id, source, answer '''
    ordered_data = [0,0,0]
    for cr in data:   # Select a similarity
        if cr[0] == "id":
            ordered_data[0] = cr[1]
        elif cr[0] == "source":
            ordered_data[1] = cr[1]
...
def login():
    if request.method == 'POST':
        ...
        if app_info["username"] == "":
            ...
            app_info["username"] = "That username does not exist. Please register."
            app_info["logged"] = False
            return redirect(url_for('index'))
...
def game():
    ...
    if app_info["game"] == False:
        ...
        all_similarities = json.loads(read_from_file("similarities.json"))

        for x in range(0, 10):  # Select 10 images at random
            repeat = True
            while repeat:
                choose_game=random.choice(all_similarities)
                if choose_game.items() not in current_game:
                    repeat = False
            current_game.append(choose_game.items())

        current_similarity = sort_current_similarity(current_game[similarity_counter])
    ...
    if request.method == 'POST':
        if 'play' in request.form:
            ...
            if attempt == 1:
                ...
                if answer.lower() in current_similarity[2].lower().replace("_", " "): # answer correct
                    ...
                    similarity_counter += 1         # Next similarity
                    if similarity_counter > len(current_game)-1:    # If that was last similarity then
                        # store_game_info()
                        return redirect(url_for('game_over'))   # GAME OVER
                    # Trigger next similarity
                    current_similarity = sort_current_similarity(current_game[similarity_counter])
                ...
                answer = ""
                index = ""
                local_answer = current_similarity[2].split()
                ...
                if answer.lower() in current_similarity[2].lower().replace("_", " "): # Answer correct
                    ...
                    if similarity_counter > len(current_game)-1:
                        # store_game_info()
        ...
        # increase attempt
            ...
            if attempt == 1:
                ...
                if similarity_counter > len(current_game)-1:     # Call next similarity
                    return redirect(url_for('game_over'))
                current_similarity = sort_current_similarity(current_game[similarity_counter])

    return render_template("game.html", app_info=app_info, all_similarities=all_similarities, current_game=current_game, current_similarity=current_similarity, similarity_counter=similarity_counter+1, attempt=attempt, points=points, gained_points=gained_points, wrong_answers=wrong_answers)
